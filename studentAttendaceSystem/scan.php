...
$result = $db->query("SELECT * FROM attendance WHERE student_id = $student_id AND date = '$date'");
...
if ($record) {
  if ($record['present']) {
    ...
    $status = 'Your attendance has been updated to present';
    $db->query("UPDATE attendance SET present = 1 WHERE student_id = $student_id AND date = '$date'");
  }
} else {
